...
BuildOrderManagerClass::BuildOrderManagerClass(bool startPaused)
	: mCurrentBuild(BuildOrderID::None)
	, mFinishedBuild(false)
	, mBuildFinishTime(0)
	, buildPaused(startPaused)
{
}

void BuildOrderManagerClass::changeCurrentBuildEvent(IEventDataPtr evnt)
{
	// Cast to correct eventtype
	std::tr1::shared_ptr<ChangeBuildOrderEvent> pEventData = std::tr1::static_pointer_cast<ChangeBuildOrderEvent>(evnt);
	BuildOrderID buildOrder = pEventData->mBuildOrder;
...
void BuildOrderManagerClass::changeCurrentBuild(BuildOrderID ID)
{
	...
	if(ID != BuildOrderID::Finished && !buildPaused)
	{
		mFinishedBuild = false;
		mItemsWaiting.clear();
...
void BuildOrderManagerClass::addBuildItem(const BuildItem &item)
{
	...
	if(ptr)
	{
		...
		TaskManager::Instance().addTask(ptr);
	}
	else
	{
