...
void BuildingPlacerClass::finaliseReservations()
{
	...
	for each(Chokepoint choke in TerrainAnalysis::Instance().getChokepoints())
	{
...
public:
	...
	{
		return BuildingPlacer::Instance().isTileWalkable(location, mIgnoreReservations) && TerrainAnalysis::Instance().getRegion(location) == mRegion;
	}
...
bool BuildingPlacerClass::isLocationNonBlocking(TilePosition position, BWAPI::UnitType type, bool ignoreReservations) const
{
	...
	Region region = TerrainAnalysis::Instance().getRegion(position);
	...
	for each(Unit unit in UnitTracker::Instance().getAllUnits())
	{
		...
		if(UnitHelper::unitProducesGround(unit->getType()))
		{
			...
			{
				if(TerrainAnalysis::Instance().getRegion(tile) == region)
				{
