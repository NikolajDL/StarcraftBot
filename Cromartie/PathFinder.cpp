...
public:
	...
	bool operator()(TilePosition position)
	{
		...
		return !mStickToRegion || TerrainAnalysis::Instance().getRegion(position) == TerrainAnalysis::Instance().getRegion(mStart);
	}
...
PositionPath PathFinderClass::CreateCheapWalkPath(Position start, Position target)
{
	...
	Region startRegion = TerrainAnalysis::Instance().getRegion(TilePosition(start));//TODO: explore why there may be a null region returned from this
	Region endRegion = TerrainAnalysis::Instance().getRegion(TilePosition(target));
