...
TerrainAnalysisClass::TerrainAnalysisClass()
{
}

void TerrainAnalysisClass::analyseBasesAndTerrain()
{
	...
	bool operator()(TilePosition location) const
	{
		for(int x = location.x()-1; x <= location.x()+1; ++x)
		{
			for(int y = location.y()-1; y <= location.y()+1; ++y)
			{
				...
				if(!tile.isValid() || TerrainAnalysis::Instance().getRegion(tile) != mRegion || !BuildingPlacer::Instance().isTileWalkable(tile))
					return false;
			}
		}
...
void TerrainAnalysisClass::finaliseConnectivity()
{
...
void TerrainAnalysisClass::calculateWalkTileClearance()
{
...
std::pair<WalkPosition, WalkPosition> TerrainAnalysisClass::findChokePoint(WalkPosition center)
{
...
void TerrainAnalysisClass::calculateConnectivity()
{
...
void TerrainAnalysisClass::createRegions()
{
	mMapWidth = BWAPI::Broodwar->mapWidth() * 4;
	mMapHeight = BWAPI::Broodwar->mapHeight() * 4;
...
void TerrainAnalysisClass::createBases()
{
...
void TerrainAnalysisClass::removeChokepoint(Chokepoint chokeToRemove)
{
