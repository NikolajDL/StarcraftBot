...
void UnitTrackerClass::pumpUnitEvents()
{
	...
	// Event handlers
void UnitTrackerClass::onUnitDiscoverEvent(IEventDataPtr evnt)
{
	// Cast to correct eventtype
	std::tr1::shared_ptr<UnitDiscoveredEvent> pEventData = std::tr1::static_pointer_cast<UnitDiscoveredEvent>(evnt);
	BWAPI::Unit* unit = pEventData->m_Unit;

	onUnitDiscover(unit);
}
void UnitTrackerClass::onUnitDestroyEvent(IEventDataPtr evnt)
{
	// Cast to correct eventtype
	std::tr1::shared_ptr<UnitDestroyedEvent> pEventData = std::tr1::static_pointer_cast<UnitDestroyedEvent>(evnt);
	BWAPI::Unit* unit = pEventData->m_Unit;
